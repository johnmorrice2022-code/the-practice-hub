...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (error) {
      toast({ title: "Could not log in", description: error instanceof Error ? error.message : String(error), variant: "destructive" });
    }
  };
        ...
        <div className="w-full max-w-md">
          <div className="bg-card rounded-xl p-10 card-shadow border border-border">
            ...
            <p className="text-sm text-muted-foreground text-center mb-8">
              Log in to continue your <span className="text-accent-amber">Jam Sessions</span>
            </p>

            <button
              onClick={handleGoogleLogin}
              className="w-full h-11 rounded-lg border border-border bg-background text-foreground text-sm font-medium hover:bg-secondary transition-colors flex items-center justify-center gap-2 mb-6"
            >
