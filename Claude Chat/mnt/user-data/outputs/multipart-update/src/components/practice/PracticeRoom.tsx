...
import { ChevronLeft, ChevronRight, X, Loader2, Sparkles, Send } from "lucide-react";
...
export function PracticeRoom({ config, onExit }: PracticeRoomProps) {
  ...
  const handlePartAnswerChange = (partLabel: string, value: string) => {
    ...
    setPartAnswers((prev) => ({
      ...prev,
      [currentQuestion.id]: { ...(prev[currentQuestion.id] || {}), [partLabel]: value },
    }));
  };
  ...
  const buildAnswerForMarking = (q: Question): string => {
    ...
    if (isMultiPart) {
      ...
      return q.parts
        .map((p) => `Part (${p.part_label}): ${parts[p.part_label] || "(no answer)"}`)
        .join("\n\n");
    }
    return answers[q.id] || "";
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="sticky top-0 z-10 bg-background border-b border-border/60">
        <div className="max-w-[720px] mx-auto px-6 h-11 flex items-center justify-between">
          <span className="text-xs text-muted-foreground tracking-wide">
            ...
            {phase === "review" ? `${totalAwarded}/${totalAvailable} marks` : `${totalMarks} marks total`}
          </span>
          <div className="flex items-center gap-3">
            {phase === "answering" && (
              <button onClick={generateAIQuestions} disabled={generatingQuestions} className="text-xs text-muted-foreground/60 hover:text-primary transition-colors flex items-center gap-1" title="Generate new questions">
                <Sparkles size={12} /> New set
              </button>
            )}
            <button onClick={onExit} className="text-muted-foreground/60 hover:text-foreground transition-colors p-1" aria-label="Exit session">
              <X size={16} />
            </button>
          </div>
        </div>
      ...
      <div className="max-w-[720px] mx-auto px-4 sm:px-6 py-10">
        <div className="bg-card rounded-xl p-8 sm:p-10" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
          {phase === "review" && currentFeedback ? (
            <FeedbackCard feedback={currentFeedback} questionNumber={currentIndex + 1} />
          ) : phase === "marking" && isMarking ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <Loader2 className="h-5 w-5 animate-spin text-primary" />
              <p className="text-sm text-muted-foreground">Marking question {currentIndex + 1}…</p>
            </div>
          ) : (
            <>
              <QuestionCard
              ...
              {phase === "answering" && currentQuestion && hasAnswer(currentQuestion) && !feedbacks[currentQuestion.id] && (
                <div className="mt-6 flex justify-end">
                  <button onClick={() => markAnswer(currentQuestion.id)} disabled={!!markingId} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors disabled:opacity-40">
                    {markingId === currentQuestion.id ? <Loader2 size={12} className="animate-spin" /> : <Send size={12} />}
                    Mark this question
                  </button>
                </div>
              )}
              ...
              {phase === "answering" && currentQuestion && feedbacks[currentQuestion.id] && (
                <div className="mt-8 pt-6 border-t border-border/40">
                  <FeedbackCard feedback={feedbacks[currentQuestion.id]} questionNumber={currentIndex + 1} />
                </div>
              )}
            </>
          )}
        ...
        <div className="flex items-center justify-between mt-8 px-1">
          <button onClick={() => goTo(currentIndex - 1)} disabled={currentIndex === 0} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-default transition-colors py-1">
            <ChevronLeft size={14} /> Previous
          </button>

          <div className="flex items-center gap-1.5">
            {questions.map((q, i) => (
              <button key={q.id} onClick={() => goTo(i)} className={`rounded-full transition-all duration-200 ${
                i === currentIndex ? "w-2.5 h-2.5 bg-foreground/50"
                : feedbacks[q.id] ? `w-1.5 h-1.5 ${feedbacks[q.id].marks_awarded / feedbacks[q.id].marks_available >= 0.7 ? "bg-[hsl(var(--success))]" : "bg-primary"}`
                : hasAnswer(q) ? "w-1.5 h-1.5 bg-foreground/25"
          ...
          {currentIndex < questions.length - 1 ? (
            <button onClick={() => goTo(currentIndex + 1)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors py-1">
              Next <ChevronRight size={14} />
            </button>
          ) : phase === "answering" ? (
            <button onClick={handleFinish} disabled={!allAnswered} className="text-xs text-primary hover:text-primary/80 transition-colors py-1 disabled:opacity-30 disabled:cursor-default">
              Finish & mark all
            </button>
          ) : (
            <button onClick={onExit} className="text-xs text-primary hover:text-primary/80 transition-colors py-1">
              Done
            </button>
          )}
        </div>
