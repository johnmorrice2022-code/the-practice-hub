...
- [x] **Phase 3 in-place conversion — PILOT DONE 25/06/2026** (Specific Heat Capacity: 11 converted / 5 skipped / 0 dropped, all verified). ⚠️ **John: confirm Verify JWT is OFF for `convert-legacy-to-stepped`**, then review→approve→publish the 11 SHC conversion drafts (publishing updates each live question in place). Then run "Convert calc → stepped" on Internal Energy, Wave Properties, Series/Parallel.
- [ ] **Phase 3 rollout** to the other calc subtopics; then `select_steps` *generation* (generator is calc-only); then Phase 4 Edexcel Maths `numeric_single` / `numeric_with_working`.
