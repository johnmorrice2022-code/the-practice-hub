...
- **Not built yet:** finish Phase 3 rollout (Internal Energy, Wave Properties, Series/Parallel); `select_steps` *generation* (the generator is calc-only so far); Phase 4 (Edexcel Maths `numeric_single` / `numeric_with_working`). Recall/explain legacy questions (all 39 Circuit Symbols etc.) stay AI-marked until select_steps generation exists.
...
- [ ] **Deliverable 2 — stepped INSIDE multi-part (next).** Multi-part calcs (~6, mostly Series/Parallel V=IR chains) must keep their authentic exam structure (shared stem/diagram, parts a/b/c, each marked independently) with a stepped scaffold *inside* each calc part — NOT collapsed into one long question (John: collapsing breaks authenticity). Needs per-part `answer_model`+`steps` (data model), player, converter, editor.
