...
- **Not built yet (next sessions, in order):** PracticeRoom step player (wrong step → pre-written hint → "I'm stuck" escalates to JAM Help with that step's context) → generator edge function that proposes steps+distractors into the Review Queue. Nothing in the **student** app reads `answer_model`/`steps` yet — practice behaviour unchanged; only the admin Review Queue now reads/writes them.
**Flag still open:** the input layer is free-text-only today (`MathEditor`); the step player needs new tile/slot + numeric+unit inputs. The legacy AI-marking path (`mark-answer`) stays the default for `ai_freeresponse` until stepped questions exist to switch to.
**To author the pilot now:** in Review Queue, open any pending P=IV question, press **E**, set Answer model → Stepped calculation, tap "Start from the P = I × V example", adjust, Save & approve. (A pending question must exist first — generate one, or it can be seeded.)
