...
### Content
- [ ] Generate review-queue questions for `completing-the-square` (prompt_config now has marking_guidance + few-shot examples for both a=1 and a>1, ready for Content Pipeline → Review Queue) — review generated `a>1` questions carefully on first run to confirm the new method block is being followed correctly.
