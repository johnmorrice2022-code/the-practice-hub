...
- **Not built yet:** `select_steps` *generation* (the generator is calc-only so far); Phase 4 (Edexcel Maths `numeric_single` / `numeric_with_working`). Recall/explain legacy questions (all 39 Circuit Symbols etc.) stay AI-marked until select_steps generation exists.
