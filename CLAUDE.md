...
- **`select_steps` UI — DONE 25/06/2026.** `SelectStepsView` in `SteppedPlayer` (a single-`select_steps` question — extended-response / required-practical 6-marker — renders a tappable checklist with a partial-marks **Submit**, no advance-gating). `buildSelectStepsReveal` (pure + tested) maps the correct points in `order` (hit/missed) plus flagged wrong picks onto the FeedbackCard `step_breakdown` (§7); `completeStepped` records the partial total; FeedbackCard now hides the Worked-solution block when empty. Review Queue: `SelectStepsFields` in `SteppedQuestionEditor` authors it (max marks, statements with correct/order/distractor) + preview. **41 vitest tests.**
- **Phase 2 generator — DONE 25/06/2026.** `supabase/functions/generate-stepped-questions` proposes `stepped_calculation` DRAFTS (choose-equation → substitute → numeric, with misconception distractors + pre-written hints) into `pending_questions`. Physics-calculation focused; "Both" tier splits Foundation + Higher (Higher = rearrangement / multi-equation chains / unit conversion). Structural validation drops malformed drafts (slot-in-expression, ends-with-numeric, one correct option) and reports the count; numeric strings coerced defensively. Triggered by the **"Generate 6 stepped"** button on Physics subtopics in the Review Queue. **Deployed `--no-verify-jwt` 25/06/2026 → confirm Verify JWT is OFF in the dashboard.** Verified live against Electrical Power and Energy Transfers. The drafts flow through the existing Review Queue stepped editor/preview/publish path unchanged.
- **Phase 3 — convert legacy AI-marked calc questions to stepped, IN PLACE (PILOT DONE 25/06/2026).** `supabase/functions/convert-legacy-to-stepped` reads a subtopic's live AI-marked **single-answer** questions and authors a stepped scaffold for each **calculation** using the question's OWN numbers (its `worked_solution` is the source of truth); non-calcs (explain/recall/multi-part) are skipped. Drafts land in `pending_questions` tagged with a new **`source_question_id`** column (uuid FK→`questions`, migration `20260625120000`, applied via `db push`). The Review Queue has a green **"Convert calc → stepped"** button (Physics) + a **"Converts live ✓ in place"** badge; `handlePublish` branches so a draft with `source_question_id` **UPDATEs the original live `questions` row** (answer_model→`stepped_calculation`, steps→scaffold, stale AI mark-scheme/worked-solution cleared) rather than inserting a duplicate. **Pilot: Specific Heat Capacity — 16 candidates → 11 converted / 5 skipped / 0 dropped; all link to a live row, 12/12 arithmetic links verified against the originals.** The 11 drafts await John's review → approve → publish (does the in-place swap). **Deployed `--no-verify-jwt` → confirm Verify JWT OFF for `convert-legacy-to-stepped`.**
- **Deliverable 2 — multi-part stepped — DONE 25/06/2026.** Multi-part calc questions get per-part deterministic checking. `MultiPartSteppedView` in PracticeRoom renders shared stem + diagram, then each part as either `SteppedPlayer` (calc, with `compact` prop) or `MathEditor` (non-calc). `steppedPartResults` state tracks per-part completion; hybrid marking sends only non-stepped parts to AI, merges with deterministic results. `SteppedPlayer` now renders diagrams via `diagramComponent`/`diagramParams` props. Converter handles multi-part (per-part scaffolds). Review Queue shows per-part "Stepped"/"AI marked" badges. `SubstituteInput` renders the full expression as one KaTeX block (fixes `\frac` rendering); `\frac` forbidden in substitute `expression` fields (use `\div`). **The stepped system is generic across all Physics subtopics — no code hardcoded to specific subtopics.**
- **`select_steps` two-attempt ordering + Scaffold for ai_freeresponse — DONE 28/06/2026.** Decision: AI marking stays for **Explain/State/Show/Prove** questions (writing the response IS the exam skill — a checklist would test recognition, not construction), but two supports were added:
  - **`select_steps` now grades order, with two attempts.** Tap-in-sequence UI (`SelectStepsView` in `SteppedPlayer`) — tapping a statement numbers it (1, 2, 3…); tap again to remove + renumber. Attempt 1 shows formative feedback only ("X/6 correct" + order right/wrong, no marks) and a "Try again" button. Attempt 2 awards marks: `clamp(correct − wrong, 0, maxMarks)` then **−1 if the correct statements aren't in canonical `order`** (so 6 correct in the wrong order scores 5). `checkSelectSteps` returns `orderCorrect: boolean` (ascending canonical `order` among the student's tap sequence, ignoring distractors). `buildSelectStepsReveal` now returns **only the correct statements in order** (distractors dropped from the mark-breakdown reveal — they're noise once marks are settled). **41 vitest tests.**
  - **Scaffold (new, jsonb column on `questions`+`pending_questions`, migration `20260628120000`):** `{ vocabulary: string[], sentence_starter: string }` — a static "Need a hand?" reveal for nervous students who aren't ready to articulate a JAM Help question. **No AI call at render time** — pre-written, reviewed before publish, same trust model as everything else in the deterministic system. `generate-pending-questions` now drafts a scaffold for every `ai_freeresponse` question/part (explicitly excluded for Physics calc parts, which get the deterministic stepped scaffold instead) — **deployed `--no-verify-jwt` 28/06/2026, confirm Verify JWT OFF**. Review Queue: scaffold view panel + JSON edit textarea for non-stepped questions, carried through `handleSaveEdit` (audit log) and `handlePublish` (both the fresh-insert and in-place-conversion branches). `PracticeRoom`: a "Need a hand?" toggle button next to JAM Help (only shown when `scaffold` is set) reveals vocabulary chips + an italic sentence starter — single-part `ai_freeresponse` questions only so far; **multi-part non-stepped scaffold display is not yet wired** (each part *can* carry a `scaffold` from generation, but `MultiPartSteppedView` doesn't render it per-part yet).
- **Not built yet:** `select_steps` *generation* (the generator is calc-only so far); per-part Scaffold display in multi-part questions; Phase 4 Maths (see "Next — Phase 4: Maths marking strategy" in Current Priorities below — the plan is now more nuanced than a blanket `numeric_single`/`numeric_with_working` conversion). Recall/explain legacy questions (all 39 Circuit Symbols etc.) stay AI-marked — that's now the intended permanent design, not a gap, per the 28/06/2026 decision above.
...
### Immediate next session(s) — Physics validation before Maths (John's call, 30/06/2026)
**No more new features until Physics is tested extensively.** John is spending the next working session(s) on:
- [ ] Run **"Convert calc → stepped"** on remaining legacy Physics calc questions (Wave Properties and Internal Energy haven't been converted at all yet; ~17 candidates total flagged by the heuristic in the cleanup session below).
- [ ] Generate fresh batches across Physics subtopics and review quality under the current (post-cleanup, post-scaffold-hardening) prompts.
- [ ] **Scrutinise JAM Help** (`supabase/functions/jam-help/`) — flagged as the next thing needing the same rigour just applied to Scaffold. It's still pure prompt-engineered AI coaching with no deterministic backstop; the "never delivers a verdict" promise rests entirely on prompt discipline there, unlike the `leaksMarkScheme()` safety net now protecting Scaffold. Not urgent, but the next quality-hardening target.
- Maths Phase 4 planning is written up below but **deliberately not started** until the above is done.
...
- **New `backfill-scaffold` edge function** (`supabase/functions/backfill-scaffold/`, deployed `--no-verify-jwt` — **confirm Verify JWT OFF**) drafts the `scaffold` field for EXISTING, already-published `ai_freeresponse` questions, **without** reopening their question_text/mark_scheme/worked_solution for re-review (additive-only field, updates the live `questions` row directly). Single-part questions only (mirrors the Scaffold display gap — multi-part isn't wired into `MultiPartSteppedView` yet).
- **First backfill pass had two real quality bugs, caught on review and fixed (full detail: STEPPED_QUESTIONS.md §8 "Quality hardening"):**
  1. Pure recall questions ("Name component X", "State the unit of...") got useless generic filler scaffolds — no diagram context, nothing to actually scaffold. Fixed: prompt now explicitly skips these (`{"skip":true}`).
  2. A diagram-dependent comparison question's sentence starter **leaked a mark scheme criterion verbatim** ("Wave B has a shorter wavelength than Wave A" — itself one of the marks). Prompt instructions alone weren't reliable enough, so added a **deterministic safety net** — `leaksMarkScheme()` checks every candidate sentence_starter for substring/4-word-window overlap against every mark scheme criterion (not just the conclusion) and strips the scaffold to null if it matches. Applied to **both** `backfill-scaffold` and `generate-pending-questions` (the latter has higher leak risk — same model call authors the mark scheme and the scaffold).
- **Final state after the hardened re-run:** 38 single-part questions genuinely scaffolded (good quality, spot-checked), the rest correctly skipped (pure recall) or dropped (would have leaked — no scaffold beats a leaking one). Live Physics stock: 135 questions — 38 stepped, 38 free-response with scaffold, 59 free-response without (mostly correctly-skipped recall + a few multi-part). ~17 still "look like" calculations and are candidates for John's **"Convert calc → stepped"** button per subtopic.
- This is now a clean, quality-checked baseline before starting Phase 4 Maths deterministic marking. **Lesson carried into Phase 4:** don't trust an LLM's self-restraint on "don't reveal X" for content it just authored — verify deterministically.
...
- **`select_steps` two-attempt flow with order grading.** Tap-in-sequence UI replaces checkboxes — tapping a statement numbers it (1, 2, 3…), tap again to remove + renumber. Attempt 1 is formative-only ("X/6 correct" + order right/wrong, no marks, "Try again"); attempt 2 awards `clamp(correct − wrong, 0, maxMarks)` then **−1 if the correct statements aren't in canonical order**. `checkSelectSteps` returns `orderCorrect`; `buildSelectStepsReveal` now shows only the correct statements (distractors dropped from the mark-breakdown reveal). Full detail + the why: STEPPED_QUESTIONS.md §6.
- **Scaffold (new feature) for `ai_freeresponse` questions** — a static "Need a hand?" reveal (vocabulary chips + a sentence-starter) for students who can't yet articulate a JAM Help question. **No AI call when the student opens it** — drafted by the generator alongside mark_scheme, reviewed before publish. New `scaffold` jsonb column on `questions`/`pending_questions` (migration `20260628120000`). `generate-pending-questions` deployed `--no-verify-jwt` 28/06/2026 — **confirm Verify JWT OFF**. Review Queue can view/edit it; `PracticeRoom` shows the toggle next to JAM Help. **Single-part only so far** — multi-part non-stepped scaffold display not yet wired into `MultiPartSteppedView`. Full detail: STEPPED_QUESTIONS.md §8.
- 41 vitest tests pass; typecheck + production build clean throughout.
...
- **`select_steps`** (deterministic 6-markers): **player checklist + §7 reveal + Review Queue editor all shipped 25/06/2026.**
...
- [x] **`select_steps` UI — DONE 25/06/2026** — player checklist + §7 ordered reveal + Review Queue editor. (See Architecture Patterns → Marking philosophy.)
- [x] **Phase 2 generator — DONE 25/06/2026** — `generate-stepped-questions` edge function + "Generate 6 stepped" Review Queue button, deployed `--no-verify-jwt`. ⚠️ **John: confirm Verify JWT is OFF for `generate-stepped-questions` in the Supabase dashboard** (it must be off — it's called with the anon key from the client, like the other generate functions).
- [ ] **Smoke-test artifacts:** my count=2 test run left 2 stepped drafts in `pending_questions` for Electrical Power and Energy Transfers (batch `b48d3f53-…`). Review or delete in the Review Queue.
- [ ] Clean up / keep the 4 earlier test stepped questions in Electrical Power and Energy Transfers (live `questions` rows) as desired.
- [x] **Phase 3 in-place conversion — ROLLED OUT 25/06/2026.** 27 single-answer calc questions converted to stepped drafts across 4 subtopics (SHC 11, Internal Energy 8, Wave Properties 6, Series/Parallel 2), all verified (27/27 source links resolve, 30/30 arithmetic correct vs the originals). ⚠️ **John: confirm Verify JWT OFF for `convert-legacy-to-stepped` + `generate-stepped-questions`, then review→approve→publish the 27 drafts** (publishing updates each live question in place). Circuit Symbols skipped (no calcs).
- [x] **One-final-answer rule — DONE 25/06/2026 (STEPPED_QUESTIONS.md "One final answer per calculation").** A stepped calc now has EXACTLY ONE numeric step; intermediate values (Δθ, unit conversions, P-before-E) are folded into the substitute slot values (+ distractor tiles), and the generator/converter author a stored `worked_solution` (line-per-step breakdown) + `mark_scheme` (AQA 1-mark-per-step) shown on the mark screen + Review Queue. Validator enforces it (lib + both edge fns). All 26 SHC/Internal Energy/Wave/Series-Parallel conversions re-run under this model: 26/26 one answer, 26/26 carry the breakdown, 26/26 arithmetic correct, all link in place. **John: review→publish the 26.**
- [x] **Deliverable 2 — stepped INSIDE multi-part — DONE 25/06/2026.** Multi-part calc questions now get per-part deterministic checking: calc parts render `SteppedPlayer` (answer-first + JAM Help + "Provide stepped help"); non-calc parts stay AI-marked. `MultiPartSteppedView` in PracticeRoom, `SteppedPlayer` `compact` prop + diagram rendering, converter handles multi-part (per-part scaffolds), Review Queue per-part stepped/AI badges, `handlePublish` carries `parts`. Substitute expression LaTeX fix: full expression rendered as one KaTeX block; `\frac` forbidden in substitute `expression` fields (use `\div`). 5 multi-part conversions for Series/Parallel (3 Foundation, 2 Higher). **The stepped system is now generic across all Physics subtopics — no code hardcoded to specific subtopics.**
- [x] **Post-Deliverable 2 polish (25/06/2026):** SteppedPlayer now renders diagrams (was missing — circuit diagrams etc. now show on stepped questions). `choose_equation` options shuffled randomly so the correct answer isn't always first. Backfilled tier on all 115 null-tier pending questions across 5 subtopics (marks-based heuristic: ≤3m → Foundation, ≥4m → Higher; Completing the Square → all Higher). Attached missing `circuit-diagram` to the 7.2V series stepped question. Deleted a poorly worded 1-mark "state + give a reason" question. **Tier filtering in PracticeRoom** — for "Both" subtopics, `loadQuestions` now fetches the student's tier from their profile and filters questions so Foundation students only see Foundation-tagged questions and Higher students only see Higher. Questions with `null` tier (pre-tagging) are shown to all as a fallback.
- [ ] Then `select_steps` *generation* (generator is calc-only); then Phase 4 Edexcel Maths `numeric_single` / `numeric_with_working`.

### Next — Phase 4: Maths marking strategy (PLANNING ONLY, 30/06/2026 — do not start building yet)
**Not starting until Physics is tested extensively** (John is spending the next session converting remaining legacy Physics calc questions + generating/reviewing fresh batches to validate quality — JAM Help scrutiny is the other pre-Maths priority, see below). When Maths work does start, the plan has shifted from "convert everything to deterministic" to a more targeted strategy, worked out 30/06/2026:

**Why full determinism doesn't transfer cleanly from Physics.** 26 active Maths subtopics (vs 9 Physics) split into answer *shapes* very differently from Physics' "pick equation → substitute → one number":
- **Single number** (Percentages, Compound Interest, Direct/Inverse Proportion, Ratio Problems, Solving Linear Equations, parts of Probability) — closest to the existing Physics pattern, cheapest to convert.
...
- **Expression as the answer** (roughly half of Algebra — Factorising, Expanding, Rearranging Formulae, Algebraic Fractions, Sequences nth term, Completing the Square) — needs a genuinely new expression-equivalence checker (is `3x(x+2)` the same as `3x²+6x`? is `x(x+5)` accepted but not `1x(x+5)`?). This is the long pole — nothing like it exists yet, and it blocks the single biggest topic.
- **Multi-method reasoning** (Circle Theorems and similar) — **deliberately NOT being forced into one deterministic chain.** A question may have several genuinely different valid theorem chains to the same answer; prescribing one path would mark a correct alternative method wrong. Staying AI-marked here is now the *intended permanent design*, mirroring the Physics Explain/Describe decision (#7) — not an incomplete conversion.
- **Genuine prose** ("show that"/"prove that") — already the locked exception (decision #6), needs its own UX, not a reuse of the Physics scaffold pattern.

**Revised plan when work starts:**
1. **Universal Scaffold rollout for Maths first** — same low-risk, additive pattern already built and hardened for Physics (skip pure recall, no question-restating vocabulary, deterministic `leaksMarkScheme()` safety net). Doesn't touch marking at all, so it's safe to ship ahead of any marking-model decisions.
2. **Audit Maths mark schemes for alternative-method coverage** — the `"Method 1: X OR Method 2: Y"` pattern already exists (Seeded question mark scheme format, above) but needs checking it's actually used everywhere a genuine alternative method exists, especially Circle Theorems. A mark scheme that only encodes one path is the biggest cause of an AI marker unfairly penalising a correct answer.
3. **Build a marking-quality eval harness** for multi-method topics — not a deterministic unit test (it's judging prose/reasoning), but a repeatable structured check: a set of question + multiple genuinely-different-but-correct student answers per topic, run through `mark-answer`, scores reviewed by John. Re-run whenever the marking prompt changes. Same spirit as the vitest suite, human-in-the-loop instead of pass/fail.
4. **Hybrid model for topics like Circle Theorems** — when a question ends in one number (the angle), check that value deterministically as a backstop even while the reasoning/method marks stay AI-judged. Narrows the AI's job to just the part that needs judgment, without forcing one theorem chain.
5. **Single-number topics convert via existing patterns** (`numeric_single` for answer-only, the existing `stepped_calculation` shape for working-required) — these are the cheap, low-risk wins, pilot first to confirm no Maths-specific gotchas before the rest.
6. **Expression-equivalence checker** is its own dedicated build — pilot on one clean case (e.g. Factorising Quadratics) before generalising to the rest of Algebra.
