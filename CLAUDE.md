...
- **Adaptive answer-first modes — DONE 24/06/2026 (STEPPED_QUESTIONS.md §5/§7).** `SteppedQuestion` gained `default_mode` (`'direct'|'stepped'`, else tier-derived: Higher→direct) and `show_givens` (else tier-derived: Higher hides the given chips). `SteppedPlayer` now offers **Direct mode** (answer the final value first; correct → full marks; wrong → a deterministic `numericDistractorHint` then a "Break it down" button that unfolds the scaffold) and **Stepped mode** (guided, with a "Let me just answer" override). On completion, `buildWorking()` assembles equation→substitution→answer from the steps into the `worked_solution` so the **full working shows on the mark screen on every path** (Physics rewards working). Editor exposes entry-mode + show-givens + per-numeric-step distractor hints. New step kind **`select_steps`** (deterministic 6-markers) has its pure checker + tests; its UI is the remaining piece. The seeded P=IV question (live `questions` row) now carries 3 misconception distractors.
