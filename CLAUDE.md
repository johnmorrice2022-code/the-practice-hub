...
- **Not built yet:** `select_steps` *generation* (the generator is calc-only so far); per-part Scaffold display in multi-part questions; Phase 4 (Edexcel Maths `numeric_single` / `numeric_with_working`). Recall/explain legacy questions (all 39 Circuit Symbols etc.) stay AI-marked — that's now the intended permanent design, not a gap, per the 28/06/2026 decision above.
...
### Next session — Phase 4: Edexcel Maths deterministic marking
The stepped system is complete for Physics. Next: bring deterministic marking to Edexcel Maths.
- [ ] **`numeric_single`** — answer-only Maths questions (single number/expression, no working required). Direct numeric check, no scaffold needed. Covers many Foundation questions and shorter Higher questions.
- [ ] **`numeric_with_working`** — "show that"/"prove that" and calculation questions where working is expected. Needs a different UX from the Physics scaffold (Maths working is more freeform than choose-equation → substitute → answer).
- [ ] Decide which existing Maths subtopics to pilot on (e.g. `completing-the-square` has 17 pending questions).
- [ ] Generator + converter for Maths answer models.
