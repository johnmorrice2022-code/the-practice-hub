...
- [ ] **Phase 3 remaining gap — multi-part calcs (~6, mostly Series/Parallel V=IR chains) are NOT converted** (the converter only handles single-answer questions; the `SteppedQuestion` model is single-question). Decide: build multi-part stepped, split each part into its own stepped question, or leave AI-marked.
