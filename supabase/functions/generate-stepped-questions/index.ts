...
- THE SUBSTITUTE EXPRESSION MUST BE FULLY SUBSTITUTABLE: every quantity on the right-hand side MUST be a [slot]. NEVER leave a bare symbol (e.g. do not write "F = [m] \\\\times a"). If the unknown is NOT the subject of the standard formula (e.g. you are given F and m and must find a), REARRANGE first: make the correct choose_equation option the rearranged form (e.g. "a = F \\\\div m") and substitute into THAT, so the expression becomes "a = [F] \\\\div [m]". This applies at Foundation too — present the equation already rearranged for the unknown.
- NEVER use \\\\frac{}{} in the "expression" field — use \\\\div for division (e.g. "I = [V] \\\\div [R]", NOT "I = \\\\frac{[V]}{[R]}"). LaTeX fractions break the interactive slot rendering. \\\\frac IS allowed in "prompt" text and in choose_equation option "latex" fields.
...
- COMBINE multi-equation methods into ONE expression wherever clean: find E from I, V, t with "E = [I] \\\\times [V] \\\\times [t]" (one substitute, one answer), NOT P=IV then E=Pt as two answers; find F from m, v, u, t with "F = ([m] \\\\times ([v] - [u])) \\\\div [t]". One choose_equation (the combined/rearranged form), one substitute, one numeric.
